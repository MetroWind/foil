...
assert.ok(html_source.includes('id="UniformFoilControls"'));
assert.equal((html_source.match(/type="range"/g) || []).length, 4);
assert.ok(html_source.includes("B · local grating tilt"));
assert.ok(html_source.includes('id="FoilBlue" type="range" min="0"'));
assert.ok(html_source.includes('max="255" step="1" value="128"'));
...
assert.ok(demo_source.includes("this.uniform_control.disabled"));
assert.ok(demo_source.includes("this.hasControlImage()"));
assert.ok(demo_source.includes("this.controller.usesControlImage()"));
const webgl_source = fs.readFileSync("libwebgl.js", "utf8");
assert.ok(webgl_source.includes('cross_origin: "anonymous"'));
assert.ok(webgl_source.includes("Texture.constant(gl, control_color"));
